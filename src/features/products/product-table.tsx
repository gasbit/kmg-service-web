...
export function ProductTable({ products }: { products: Product[] }) {
  ...
  return (
    <>
      ...
      <Dialog description={selected ? selected.isActive ? <>สินค้านี้จะไม่สามารถเลือกใช้ในรายการใหม่ แต่ประวัติเดิมจะไม่ถูกลบ</> : <>สินค้านี้จะกลับมาเลือกใช้ในรายการใหม่ได้</> : null} dismissible={!pending} footer={<><Button data-dialog-initial-focus disabled={pending} onClick={() => setSelected(null)} variant="secondary">ยกเลิก</Button><Button isLoading={pending} loadingText="กำลังบันทึก" onClick={confirm}>{selected?.isActive ? "ยืนยันปิดใช้งาน" : "ยืนยันเปิดใช้งาน"}</Button></>} onOpenChange={(nextOpen) => { if (!nextOpen && !pending) setSelected(null); }} open={Boolean(selected)} title={selected?.isActive ? "ปิดใช้งานสินค้านี้?" : "เปิดใช้งานสินค้านี้อีกครั้ง?"} />
    </>
  );
}
