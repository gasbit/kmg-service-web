...
export function ProductLoadError({ forbidden = false, requestId }: { forbidden?: boolean; requestId?: string }) {
  ...
  return (
    <section className="grid min-h-[420px] place-items-center rounded-2xl border border-slate-200 bg-white px-6 py-14 text-center shadow-sm" role="alert">
      <div className="max-w-md">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-500">{forbidden ? "สิทธิ์การใช้งานถูกตรวจสอบโดยระบบหลังบ้าน" : "บริการสินค้าเกิดข้อผิดพลาดชั่วคราว กรุณาลองใหม่อีกครั้ง"}</p>
