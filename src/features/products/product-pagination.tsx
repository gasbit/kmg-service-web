...
function href(searchParams: Record<string, string | undefined>, page: number, limit?: number) {
  ...
  return `/products?${query.toString()}#product-results`;
}
  ...
  return (
    <div className="flex flex-col gap-4 border-t border-slate-100 px-5 py-4 text-sm text-slate-600 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {page > 1 ? <Link className="grid size-11 place-items-center rounded-lg border border-slate-200 bg-white font-bold hover:border-blue-400 hover:text-blue-600" href={href(searchParams, page - 1)} aria-label="หน้าก่อนหน้า">‹</Link> : <span aria-hidden="true" className="grid size-11 place-items-center rounded-lg border border-slate-100 text-slate-300">‹</span>}
        ...
        {page < totalPages ? <Link className="grid size-11 place-items-center rounded-lg border border-slate-200 bg-white font-bold hover:border-blue-400 hover:text-blue-600" href={href(searchParams, page + 1)} aria-label="หน้าถัดไป">›</Link> : <span aria-hidden="true" className="grid size-11 place-items-center rounded-lg border border-slate-100 text-slate-300">›</span>}
        <form action="/products#product-results" className="ml-1">
          ...
          <Select aria-label="จำนวนรายการต่อหน้า" className="h-11 min-w-24" defaultValue={String(limit)} name="limit" onChange={(event) => event.currentTarget.form?.requestSubmit()}>
